...
export default function Shop() {
  ...
  const goToPrevSlide = () => {
    setCurrentSlide((prev) => {
      const maxSlide =
        Math.ceil(featuredProducts.length / productsPerSlide) - 1;
  ...
  const fetchProductsByCategory = async (category: string) => {
    try {
      ...
      setError(
        err.message || `An error occurred while fetching ${category} products`
      );
      setLoading(false);
    }
  };
  ...
  const renderProductCard = (product: Product) => {
    ...
    return (
      <div
        key={product.id}
        className="bg-white border group duration-300 rounded-lg overflow-hidden shadow-sm hover:shadow-md transition-all group"
      >
        <Link href={`/store/${product.id}`} className="block relative">
          ...
          <Badge className="absolute top-2 left-2 capitalize" variant="outline">
            {product.category}
          </Badge>
        </Link>
        ...
        <div className="p-4">
          <div className="flex justify-between items-start mb-2">
            <Link href={`/store/${product.id}`} className="block">
              <h2 className="text-lg font-semibold hover:text-blue-600 transition-colors line-clamp-1">
                {product.name}
              </h2>
            </Link>
          ...
          <div className="flex items-center mb-3 text-sm">
            <div className="flex items-center">
              <Star
                className={`h-4 w-4 ${
                  product.avgRating > 0 ? "text-yellow-400" : "text-gray-300"
                } fill-current`}
              />
              <span className="ml-1">
                {product.avgRating > 0
      ...
      <div className="container mx-auto px-4 py-8">
        ...
        <div className="bg-blue-50 p-6 rounded-lg mb-8">
          <p className="text-gray-700 text-center text-lg">
            Find the best products for your pet! Browse our selection of food,
            toys, and medicine.
          </p>
        ...
        {!activeCategory && !searchTerm && featuredProducts.length > 0 && (
          <section className="mb-12">
            <div className="flex items-center mb-4 border-b pb-2">
              ...
              <h2 className="text-xl flex font-semibold text-blue-700">
                Featured Products
              </h2>
            </div>
            ...
            <div className="relative overflow-hidden">
              <div
                ...
                style={{
                  ...
                  display: "flex",
                  width: `${
                    Math.ceil(featuredProducts.length / productsPerSlide) * 100
                  }%`,
                }}
              >
                {featuredProducts.map((product) => (
                  <div
                    key={product.id}
                    className="w-full"
                    style={{ flexBasis: `${100 / productsPerSlide}%` }}
                  >
