    ...
    async function fetchPets() {
      try {
        ...
        if (!recommendationsResponse.ok) {
          ...
          throw new Error(
            errorData.message || "Failed to fetch pet recommendations"
          );
        }
  ...
  useEffect(() => {
    ...
    const filtered = allPets.filter((pet) => {
      ...
      return searchTags.some((searchTag) =>
        petTagsLower.some((petTag) => petTag.includes(searchTag))
      );
    });
            ...
            {pet.tags && pet.tags.length > 0 && (
              <div className="mt-2 flex flex-wrap gap-1">
                {pet.tags.slice(0, 3).map((tag, index) => (
                  <span
                    key={index}
                    className="inline-block px-2 py-1 text-xs bg-pink-100 text-pink-800 rounded-full border border-pink-200"
                  >
                    {tag}
                  </span>
                ))}
        ...
        <div className="max-w-6xl mx-auto">
          <div className="bg-pink-50 p-6 rounded-lg mb-8">
            <div className="text-center mb-8">
              <p className="text-gray-700 text-lg">
                Find your perfect companion! Browse our available pets for
                adoption.
              </p>
            </div>
            ...
            <div className="mb-8">
              ...
              {searchTerm && (
                <p className="text-sm text-gray-600 mt-2 text-center">
                  Searching for:{" "}
                  {searchTerm.split(" ").filter(Boolean).slice(0, 3).join(", ")}
                </p>
              )}
            </div>
          </div>
          ...
          {loading ? (
            ...
            <div className="text-center py-12 text-gray-500 bg-white rounded-3xl shadow-sm border-2 border-pink-200">
              No pets found matching your search. Try different tags or clear
              your search.
              <div className="mt-4">
                <Button
                  variant="outline"
                ...
                <div className="mb-10">
                  <h2 className="text-xl font-semibold mb-4 text-violet-600 flex items-center justify-center sm:justify-start">
                    ...
                    <span className="relative ">
                      ...
                      <span className="absolute -top-1 -right-2 flex h-3 w-3"></span>
                    </span>
                  </h2>
                  ...
                  <p className="mt-2.5 text-gray-500 text-center italic">
                    Note: Please change your preferences in your profile to get
