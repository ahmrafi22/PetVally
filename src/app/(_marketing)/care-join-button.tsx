...
export default function CareJoinButton() {
  ...
  return (
    <button
      ref={buttonRef}
      onClick={handleClick}
      className="relative group cursor-pointer px-8 py-4 text-lg font-bold uppercase tracking-wider text-gray-800 
                backdrop-blur-sm bg-white/25 border border-white/20 rounded-xl
                shadow-lg transition-all duration-500 overflow-hidden
                transform perspective-800 hover:scale-105 hover:rotate-1 hover:-translate-y-2
                hover:bg-gradient-to-r hover:from-green-400 hover:to-green-500
                hover:shadow-xl hover:shadow-white-500/30
    ...
    >
      ...
      <span className="relative z-20 transition-transform duration-500 transform hover:scale-110 group-hover:text-white">
        Join as a Caregiver
        <HeartHandshake className="inline-block w-8 h-7 fill-white group-hover:text-black ml-2 " />
      </span>
    </button>
  );
}
